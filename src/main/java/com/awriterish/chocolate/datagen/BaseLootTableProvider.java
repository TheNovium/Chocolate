...
public abstract class BaseLootTableProvider extends LootTableProvider {
    ...
    protected LootTable.Builder createFortuneOnlyTable(String name, Item item, float min, float max, int bonus){
        LootPool.Builder builder = LootPool.lootPool()
                .name(name)
                .setRolls(ConstantValue.exactly(1))
                .add(AlternativesEntry.alternatives(
    ...
    protected LootTable.Builder createMobLootTable(String name, Map<Item, Float> lootItems, int lootBonus){
        LootPool.Builder builder = LootPool.lootPool()
                .name(name);
        for(Item item : lootItems.keySet()){
            builder.add(
                    LootItem.lootTableItem(item)
                            .apply(SetItemCountFunction.setCount(UniformGenerator.between(0, lootItems.get(item))))
                            .apply(ApplyBonusCount.addUniformBonusCount(Enchantments.MOB_LOOTING, lootBonus))
                            .apply(ApplyExplosionDecay.explosionDecay())
            );
        }
        return LootTable.lootTable().withPool(builder);
    ...
    protected LootTable.Builder fromMobLootTable(EntityType<?> from){
        LootPoolSingletonContainer.Builder<?> originalLoot = LootTableReference.lootTableReference(from.getDefaultLootTable());
        LootPool.Builder builder = LootPool.lootPool().add(originalLoot);
        return LootTable.lootTable().withPool(builder);
    }

    @Override
    public String getName() {
        return "Chocolate LootTables";
    }
}
